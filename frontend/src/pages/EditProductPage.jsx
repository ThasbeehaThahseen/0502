...
export const EditProductPage = () => {
  ...
  const [formData, setFormData] = useState({
    images: [],
    imageColors: [], // Array to store color per image
  ...
  const loadProduct = async () => {
    try {
      ...
      // Extract image colors from images
      const imageColors = (product.images || []).map((img, index) => ({
        imageIndex: index,
        color: img.color || ''
      })).filter(ic => ic.color);
            ...
            {currentStep === 2 && (
              <div className="step-content" data-testid="step-available-colours">
                <p className="step-instruction">
                  Select color for each uploaded image. Use Auto-Detect or manually select from the dropdown.
                </p>
                  ...
                  <div className="image-carousel-container" style={{ marginTop: '20px' }}>
                    ...
                    <div style={{ marginTop: '30px', padding: '20px', border: '1px solid #e5e7eb', borderRadius: '8px', backgroundColor: 'white' }}>
                      ...
                      <div style={{ marginTop: '15px' }}>
                        ...
                        <Select 
                          value={getColorForImage(currentImageIndex)} 
                          onValueChange={(value) => setColorForImage(currentImageIndex, value)}
                        >
                          <SelectTrigger data-testid={`color-select-${currentImageIndex}`}>
                            ...
                            <div style={{ padding: '8px' }}>
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  // Filter logic handled by the select component
                                }}
